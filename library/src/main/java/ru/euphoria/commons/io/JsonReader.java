...
import ru.euphoria.commons.json.JsonScope;
import ru.euphoria.commons.json.JsonToken;


/**
 * Static version of {@link android.util.JsonReader}. Used to write apps
 * that run on JVM (Java Virtual Machine).
 *
 ...
 *
 * Each {@code JsonReader} may be used to read a single JSON stream.
 *
 * @since 1.1
 */
public final class JsonReader extends FilterReader {
     ...
     */
    private final char[] buffer = new char[EasyStreams.BUFFER_SIZE];
    private int pos = 0;
    private int limit = 0;
     ...
     *     reader is closed.
     */
    public String nextNull() throws IOException {
        peek();
    ...
    public int nextInt() throws IOException {
        ...
        try {
            ...
            System.out.println("result as double: " + (double) result);
            if ((double) result != asDouble) {
                throw new NumberFormatException(value);
            }
        }
            ...
            while (pos < limit) {
                ...
                if (c == quote) {
                    if (skipping) {
                        ...
                        return String.valueOf(buffer, start, pos - start - 1);
                    } else {
    ...
    private String nextLiteral(boolean assignOffsetsOnly) throws IOException {
        ...
        if (assignOffsetsOnly && builder == null) {
            ...
            result = String.valueOf(buffer, pos, i);
        } else {
            builder.append(buffer, pos, i);
    ...
    private char readEscapeCharacter() throws IOException {
        ...
        switch (escaped) {
            case 'u':
                ...
                String hex = String.valueOf(buffer, pos, 4);
                pos += 4;
    ...
    private JsonToken decodeLiteral() throws IOException {
        if (valuePos == -1) {
            ...
            value = String.valueOf(buffer, valuePos, valueLength);
